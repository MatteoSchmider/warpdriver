...
WarpDriver::WarpDriver(const SPIAdapter &device, MotorType motorType,
                       uint16_t numPoles, CalibrationData calibration)
    : m_device(device) {
...
uint16_t WarpDriver::getAdcRawDataI0() {
...
void WarpDriver::setPwmFrequency(uint32_t frequency) {
...
void WarpDriver::setUseSvpwm(bool on) {
  if (on)
...
void WarpDriver::initEncoder(int16_t offset) {
...
void WarpDriver::initCalibration(CalibrationData calibration) {
